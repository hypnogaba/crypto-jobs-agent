 ...
 *
 * Два кроки, і перший робить усю роботу:
 *   1. пауза одразу — доставки немає, витрат немає. Саме вона спинила 105
 *      добірок, що йшли в нікуди;
 *   2. видалення через сім днів — рядок стирається каскадом.
 *
 * Сім, а не п'ятнадцять. Спершу тут стояло п'ятнадцять: стільки живе кука
 * сесії з запасом, тож людина, яка повернеться на сайт із того самого
 * браузера, встигла б побачити попередження й підключити бота. Але за перший
 * тиждень із семи таких акаунтів не повернувся ЖОДЕН, а тримати анкету того,
 * хто жодного разу не скористався продуктом, довше за тиждень нема підстав.
 * Операційно це нічого не міняє — витрати спиняє пауза, а не видалення, —
 * тож коротший строк коштує лише шансу тих, хто повертається на восьмий день.
 *
 ...
 * свідома — і тепер її стереже тест, а не лише цей коментар.
 */
