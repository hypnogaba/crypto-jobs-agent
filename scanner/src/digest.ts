...
export function formatDigest(
  ...
  jobs.forEach((j, i) => {
     ...
     *
     * Але витяг живе МОВОЮ ОГОЛОШЕННЯ, тобто англійською, тож у картку його
     * пускаємо лише англомовним. Українська добірка з англійським абзацом —
     * це вада, яку в цьому продукті вже виправляли раніше, коли опис і
     * переклад були двома різними викликами; повертати її не можна.
     *
     * Той, хто читає іншою мовою, лишається з рядком умов (локація,
     * віддаленість, вилка) — він теж різний у кожної вакансії й теж її мовою.
     */
    const fallback = locale === "en" ? j.summary : null;
    const role = withSummaries
      ? clampSummary(tidySummary(j.roleLine ?? fallback), ROLE_LINE_MAX) : null;
