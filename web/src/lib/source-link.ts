 ...
 * одне виконання — п'ятдесят на найскромнішому тарифі. Двадцять п'ять
 * вкладається в нього навіть коли кожне посилання попросить два запити.
 *
 * Було десять, і власник раз за разом вставляв довші списки: решта мовчки
 * зникала, а людина думала, що додала все. Мовчки з'їсти більше за межу було б
 * гірше за саму межу, тож зайве лишається в полі — але межа має бути хоч
 * приблизно схожою на те, скільки посилань приносять за раз.
 *
...
/**
 * Борд Getro за сторінкою.
 *
 * jobs.solana.com, jobs.avax.network, jobs.dragonfly.xyz — усе це Getro під
 * власним доменом, і за виглядом сторінки цього не скажеш. Але номер колекції
 * лежить прямо в ній: Getro будує борди на Next.js і кладе `network.id` у
 * `__NEXT_DATA__`. Перевірено на шести бордах — усі шість віддали номер, і
 * всі шість номерів виявились живими.
 *
 * Це найцінніше, що ми вміємо додати одним посиланням: у колекції фонду
 * сотні компаній, і 80% посилань ведуть просто в ATS роботодавця.
 */
export function getroIdInPage(html: string): number | null {
  const m = /"network"\s*:\s*\{\s*"id"\s*:\s*"?(\d{1,6})"?/.exec(html);
  const id = m ? Number(m[1]) : NaN;
  return Number.isFinite(id) && id > 0 ? id : null;
}

/** Скільки в сторінці вакансій, розмічених за schema.org. */
export function jobPostingCount(html: string): number {
  return (html.match(/"@type"\s*:\s*"JobPosting"/g) ?? []).length;
}

/**
 * Чому сторінка виглядає порожньою — і що з цим робити.
 *
 * «Не розпізнано» — це не діагноз, а відмова. Власник має бачити, що саме ми
 * побачили: сторінку, яка малюється скриптом, видно за потоком RSC, а
 * заблоковану — за кодом відповіді. Різниця важлива, бо в одному випадку
 * зробити не можна нічого, а в іншому — можна.
 */
export function diagnose(html: string): { cause: string; fix: string } {
  if (/self\.__next_f\.push|__NEXT_DATA__/.test(html) && html.length > 400_000) {
    return {
      cause: "сторінка малюється скриптом — у відповіді сервера лише каркас, вакансії домальовує браузер",
      fix: "потрібен їхній API. Пошукай у них сторінку «API» або напиши їм — дошки часто дають доступ безкоштовно",
    };
  }
  if (/<script/i.test(html) && html.length > 100_000) {
    return {
      cause: "розмітки вакансій у сторінці немає: ні стрічки, ні schema.org, ні посилань на ATS",
      fix: "спробуй вставити внутрішню сторінку зі списком вакансій, а не головну — розмітка часто лише там",
    };
  }
  return {
    cause: "сторінка коротка й порожня — найпевніше це заглушка або перенаправлення",
    fix: "перевір адресу в браузері: можливо, справжній список живе за іншим шляхом",
  };
}

/** Що означає код відповіді, коли сайт нас не пустив. */
export function diagnoseHttp(note: string): { cause: string; fix: string } | null {
  if (/\b403\b/.test(note)) return {
    cause: "сайт блокує роботів (403) — нас не пускає захист від ботів, а не помилка адреси",
    fix: "з коду тут не вдіяти нічого. Варіанти: їхній офіційний API, домовитись про доступ, або взяти ті самі вакансії з дошки роботодавця",
  };
  if (/\b429\b/.test(note)) return {
    cause: "сайт просить прийти пізніше (429) — ми стукали занадто часто",
    fix: "спробуй те саме посилання за годину",
  };
  if (/\b404\b/.test(note)) return {
    cause: "такої сторінки в них немає (404)",
    fix: "перевір адресу — можливо, розділ вакансій переїхав",
  };
  if (/\b5\d\d\b/.test(note)) return {
    cause: "у них зараз щось зламано на сервері",
    fix: "не наша біда — спробуй завтра",
  };
  return null;
}

/**
 * Скільки записів вакансій лежить у потоці Next.js.
 *
 * Сторінка такої дошки виглядає порожньою: ні стрічки, ні розмітки, ні
 * посилань на ATS. Легко вирішити, що вакансії домальовує браузер, — і саме
 * так ми й вирішували. Насправді сервер уже надіслав дані для малювання, у
 * викликах `self.__next_f.push`. Так читається jobstash.xyz із його 7698
 * вакансіями, які ми чесно вважали недосяжними.
 *
 * Рахуємо грубо: питання одне — «записи там є чи ні». Розбір лишається
 * сканеру, який і читатиме цю дошку.
 */
export function nextPayloadJobs(html: string): number {
  if (!html.includes("self.__next_f.push")) return 0;
  // Пара «title + href» поруч — це рядок списку вакансій, а не будь-який
  // об'єкт сторінки. Достатньо близько, щоб не злічити сусідні поля.
  return (html.match(/\\?"title\\?":\\?"[^"\\]{4,}[\s\S]{0,400}?\\?"href\\?":\\?"\\?\//g) ?? []).length;
}
