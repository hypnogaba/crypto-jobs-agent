...
const P = {
  ...
  /**
   * ОДНА відмова на всі глухі кути прив'язки — і вона ж каже, що робити.
   *
   * Раніше тут стояло саме лише «застаріло», а сусідня гілка відповідала
   * «акаунт уже підключено». Різниця в текстах сама була відповіддю: перший
   * означав «токена не існує», другий — «токен справжній і акаунт існує». Тобто
   * сторонній, який має лише здогад, міг перевіряти здогади по одному. Тепер
   * обидві гілки шлють цей рядок, і відрізнити вигаданий токен від живого
   * чужого акаунта з відповіді бота неможливо.
   *
   * Друга половина: продукт має вхід лише через Telegram, тож людина, яка
   * втратила свій чат, не має ЖОДНОГО шляху назад — і саме вона тепер бачить
   * цей рядок. Тому в ньому названо дію: /feedback тут-таки, з нового чату.
   * Це не витік: порада однакова для всіх, і тому нічого не каже про токен.
   * Щоб порада не була брехнею, /feedback навмисно працює і в чаті без
   * акаунта (bot.ts, handleCommand).
   */
  /**
   * Одна відповідь на всі мертві посилання: вигаданий токен, протермінований,
   * уже витрачений, і токен ВХОДУ, поданий у двері прив'язки.
   *
   * Не розрізняємо їх навмисно. Сторонній, якому дістався чужий токен, не
   * мусить дізнатись зі слів бота, що токен був справжній, а акаунт існує й
   * уже має Telegram: це відрізняло б живий чужий акаунт від вигаданого.
   *
   * Поради «напиши власнику» тут немає теж навмисно, і це рішення власника:
   * порятунку акаунта в продукті НЕМАЄ. Хто втратив Telegram, той відкриває
   * бота з нового і починає з /start — новий акаунт, нова анкета, добірки
   * знову йдуть. Втрачається історія, а не продукт. Механіка «власник
   * відв'язує руками» була написана й ВИКИНУТА: сама відв'язка знімає з
   * акаунта чат, а прив'язка відмикається саме умовою «чату немає», тож той,
   * хто вже тримав перехоплену сесію, встигав перевісити акаунт на себе
   * раніше за господаря. Порятунок коштував рівно тієї діри, від якої вся ця
   * гілка й захищає.
   */
  linkExpired: {
    en: "That link does not work. Get a fresh one on the connect page.",
    uk: "Це посилання не працює. Візьми нове на сторінці підключення.",
    fr: "Ce lien ne fonctionne pas. Prenez-en un nouveau sur la page de connexion.",
    ru: "Эта ссылка не работает. Возьми новую на странице подключения.",
  },
   ...
   */
  cabinet: {
    en: "You also have a cabinet on the site: the history of every batch, your profile, the delivery hour and a pause button. The button below opens it once, and it works for 15 minutes.",
    uk: "У тебе є ще й кабінет на сайті: історія всіх добірок, анкета, година доставки й пауза. Кнопка нижче відкриє його один раз, вона діє 15 хвилин.",
    fr: "Vous avez aussi un espace sur le site : l'historique de chaque sélection, votre profil, l'heure d'envoi et une pause. Le bouton ci-dessous l'ouvre une fois, il vaut 15 minutes.",
    ru: "У тебя есть ещё и кабинет на сайте: история всех подборок, анкета, час доставки и пауза. Кнопка ниже откроет его один раз, она действует 15 минут.",
  },
  /**
   * Речення закінчене, бо під ним кнопка, а не адреса.
   *
   * Досі всі три рядки обривались двокрапкою й обіцяли посилання наступним
   * рядком, а там стояв `…/enter?token=` разом із 32 символами токена. Той
   * токен відмикає кабінет на 30 днів, і власник надіслав скриншот, з якого
   * він читався. Тепер адреса живе в inline-кнопці: зі знімка екрана її не
   * взяти. Але це не «токена більше немає»: довге натискання дає «копіювати
   * посилання», а після відкриття він видно в адресному рядку.
   */
  siteLink: {
    en: "Here is your way in. The button works once and lives 15 minutes.",
    uk: "Ось твій вхід. Кнопка спрацьовує один раз і живе 15 хвилин.",
    fr: "Voici votre accès. Le bouton ne marche qu'une fois et vit 15 minutes.",
    ru: "Вот твой вход. Кнопка срабатывает один раз и живёт 15 минут.",
  },
  /**
   * Підписи кнопок, а не речення: у кнопку Telegram влазить близько двадцяти
   * символів, далі вона розповзається на два рядки й перестає читатись як
   * дія. Три різні підписи, бо кнопка веде в три різні місця, і людина має
   * бачити, куди тисне, ще до дотику.
   */
  openSite: {
    en: "Open the site", uk: "Відкрити сайт",
    fr: "Ouvrir le site", ru: "Открыть сайт",
  },
  openCabinet: {
    en: "Open my cabinet", uk: "Відкрити кабінет",
    fr: "Ouvrir mon espace", ru: "Открыть кабинет",
  },
  openAdmin: {
    en: "Open the panel", uk: "Відкрити панель",
    fr: "Ouvrir le panneau", ru: "Открыть панель",
  },
  ...
  // Найдорожчий вхід у системі, тож і кнопка тут окрема: токен веде просто
  // в панель власника, повз будь-яку іншу перевірку, крім строку.
  adminLink: {
    en: "Owner panel. The button works 15 minutes, the session then lasts 30 days.",
    uk: "Панель власника. Кнопка діє 15 хвилин, далі сесія живе 30 днів.",
    fr: "Panneau propriétaire. Le bouton marche 15 minutes, la session dure ensuite 30 jours.",
    ru: "Панель владельца. Кнопка действует 15 минут, дальше сессия живёт 30 дней.",
  },
  cvReading: {
   ...
   *
   * Цей рядок навмисно НЕ зведено до `linkExpired`, хоча він теж каже, що
   * токен був справжній. Різниця в тому, ЧИЙ це чат: сюди можна дійти лише
   * власним чатом, у якому вже є акаунт із доставленими добірками, і сказати
   * такій людині «посилання не працює» означало б залишити її крутити цикл
   * замість того, щоб показати вихід (/site). А для стороннього ця гілка
   * недосяжна: вона стоїть ПІСЛЯ перевірки «акаунт уже має свій Telegram»,
   * тобто спрацьовує лише на акаунті без чату, який і так нікому не належить.
   */
  alreadyLinked: {
  ...
  // Ключа `accountLinkedElsewhere` тут немає навмисно, і його не варто
  // заводити знову: відмова «акаунт уже має інший Telegram» тепер говорить
  // рівно те саме, що й `linkExpired`, — інакше сторонній відрізняв би живий
  // чужий акаунт від вигаданого токена просто за текстом відповіді.
  unknown: {
