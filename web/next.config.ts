...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    serverActions: {
      /**
       * Стеля на тіло серверної дії. За умовчанням Next дає 1 МБ — і саме вона
       * ламала «завантаж CV».
       *
       * cv.ts обіцяє 4 МБ і має чемну помилку err.tooBig на все, що більше.
       * Але перевірка Next спрацьовує РАНІШЕ за саму дію: запит гинув ще до
       * startOnboarding, тож ані redirect з поясненням, ані лог не траплялись —
       * людина бачила голу сторінку 500. Резюме з фото чи версткою переходить
       * за мегабайт легко, тож ламалось воно не в рідкісному випадку.
       *
       * 5 МБ, а не 4: до самого файлу додається обгортка multipart і решта
       * полів форми, і файл рівно на межі не має падати в 500 замість
       * пояснення. Єдина межа, яку бачить людина, лишається в cv.ts.
       */
      bodySizeLimit: "5mb",
    },
